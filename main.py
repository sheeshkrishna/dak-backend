...
# Remove or comment out to avoid table creation errors
...
def create_user(user: schemas.UserCreate, session: Session = Depends(database.get_session)):
    return crud.create_user(session, user.email, user.password, auth.get_password_hash)

@app.get("/users/me", response_model=schemas.UserRead)
def read_users_me(current_user: models.Users = Depends(auth.get_current_user)):
    return current_user

@app.post("/daks/", response_model=schemas.DAKRead)
def create_dak(dak: schemas.DAKCreate, session: Session = Depends(database.get_session), current_user: models.Users = Depends(auth.get_current_user)):
